...
def main() -> int:
    ...
    agent_instructions_path = Path(__file__).resolve().parent / "opencode-local-agent.md"
    if agent_instructions_path.exists():
        instructions = config.setdefault("instructions", [])
        if not isinstance(instructions, list):
            instructions = []
            config["instructions"] = instructions
        agent_instructions_str = str(agent_instructions_path)
        if agent_instructions_str not in instructions:
            instructions.append(agent_instructions_str)
