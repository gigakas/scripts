---
name: project-review
description: Review changed code in this repo against this project's own conventions — plan-first, dead-code hygiene, layering, strict typing, changelog/commit workflow, single requirements checklist. Use for a review scoped to this project's conventions; use the generic /code-review skill for general bug-hunting.
---
...
> real ones once the project's architecture is set in `PLAN.md`, and rename the skill/file from
> `project-*` to `<project>-*` in both `.claude/skills/` and `.opencode/command/` (keep them
> mirrored).
...
- Update this file and the mirrored `.opencode/command/project-review.md` together whenever the
  rules change.
