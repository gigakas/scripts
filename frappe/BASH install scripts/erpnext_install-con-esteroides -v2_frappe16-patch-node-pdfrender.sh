...
if [[ "$bench_version" == "version-15" ]]; then
    echo -e "${YELLOW}Installing wkhtmltopdf 0.12.6.1-3 for Frappe v15...${NC}"
    sudo apt install -y xfonts-75dpi xfonts-base fontconfig libxrender1 libssl-dev
...
elif [[ "$DISTRO" == "Ubuntu" && "$wk_os_version" == "24.04" ]]; then
    echo -e "${YELLOW}Ubuntu 24.04 detected: installing wkhtmltopdf 0.12.6.1-3...${NC}"
    sudo apt install fontconfig libxrender1 xfonts-75dpi xfonts-base -y
    # Install libssl1.1 dependency required by wkhtmltopdf on Ubuntu 24.04
...
start_temp_redis() {
    ...
    port=$(redis_config_port "$config_file" "$fallback_port")
...
if [[ "$bench_version" == "version-15" || "$bench_version" == "version-16" || "$bench_version" == "develop" ]]; then
    echo -e "${YELLOW}Starting Redis instances for $bench_version (queue, socketio, and cache)...${NC}"
    sleep 1
    bench setup redis
    start_temp_redis queue 11000
    start_temp_redis socketio 12000
    start_temp_redis cache 13000
    echo -e "${GREEN}Redis instances are ready.${NC}"
    sleep 1
fi
