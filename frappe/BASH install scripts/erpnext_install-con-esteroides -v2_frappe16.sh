...
if [[ "$DISTRO" == "Ubuntu" && "$wk_os_version" == "24.04" ]]; then
...
if [[ "$bench_version" == "version-16" || "$bench_version" == "develop" ]]; then
    ...
    echo -e "${GREEN}Redis instances started on ports 11000, 12000, 13000.${NC}"
    sleep 1
fi
...
# Kill temporary Redis instances started for v16/develop install-app
