...
nginx_vhosts_file="/usr/local/lib/python${python_version}/dist-packages/bench/playbooks/roles/nginx/tasks/vhosts.yml"
sudo sed -i 's/when: nginx_vhosts$/when: nginx_vhosts | length > 0/g' "$nginx_vhosts_file"
...
case "$continue_prod" in
    "yes"|"y")
        ...
        if [ -f "$FILE" ]; then
            ...
            echo -e "${YELLOW}supervisord.conf not found, skipping ownership patch...${NC}"
        fi
